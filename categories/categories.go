...
/*ListCategory*/
func (c Category) ListCategory(limit int, offset int, filter Filter, parent_id int) (tblcat []TblCategory, categorycount int64, err error) {
	...
	if check {
		...
		err1 := GetCategoryById(&category, parent_id, c.Authority.DB)

		if err1 != nil {
			fmt.Println(err)
		}
		GetSubCategoryList(&categorys, offset, limit, Filter{}, parent_id, 1, c.Authority.DB)
